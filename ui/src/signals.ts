...
export type AsyncResult<T, E = any> = {
  status: 'pending';
} | {
  status: 'completed';
  value: T;
...
export class AsyncState<T, E = any> extends Signal.State<AsyncResult<T, E>> {
}
export class AsyncComputed<T, E = any> extends Signal.Computed<AsyncResult<T, E>> {
}

export type AsyncSignal<T, E = any> = AsyncState<T, E> | AsyncComputed<T, E>;
...
export function toPromise<T, E = any>(asyncSignal: AsyncSignal<T, E>): Promise<T> {
...
>(
  ...
  const signal = new Signal.State<AsyncResult<Array<Link>>>({ status: 'pending' }, {
    [Signal.subtle.watched]: () => {
      ...
      const fetch = () => {
        if (!active) return;
        fetchCollection().then(maybeSet).catch(e => signal.set({
          status: 'error',
          error: e
        })).finally(() => {
          if (active) {
...
export function immutableEntryStore<T>(
  ...
  let cached = false;
  const signal = new Signal.State<AsyncResult<EntryRecord<T>>>({ status: 'pending' }, {
    [Signal.subtle.watched]: () => {
      if (cached) return;
      let retries = 0;
...
>(
  ...
  const signal = new Signal.State<AsyncResult<Array<EntryRecord<T>>>>({ status: 'pending' }, {
    [Signal.subtle.watched]: () => {
      active = true;
      const fetch = async () => {
        if (!active) return;
...
>(
  ...
  const signal = new Signal.State<AsyncResult<Array<SignedActionHashed<Delete>>>>({ status: 'pending' }, {
    [Signal.subtle.watched]: () => {
      active = true;
      const fetch = async () => {
        if (!active) return;
      ...
      unsubs = client.onSignal((originalSignal) => {
        ...
        if (
          ...
          let lastDeletes = deletes ? deletes : [];
...
> {
  ...
  let deletedLinks: Array<
    [SignedActionHashed<CreateLink>, Array<SignedActionHashed<DeleteLink>>]
  > | undefined;
  const signal = new Signal.State<AsyncResult<Array<[SignedActionHashed<CreateLink>, Array<SignedActionHashed<DeleteLink>>]>>>({ status: 'pending' }, {
    [Signal.subtle.watched]: () => {
      active = true;
