...
export class CreateNotification extends LitElement {
  ...
  async createNotification(fields: Partial<Notification>) {
    if (this.notificationType === undefined) throw new Error('Cannot create a new Notification without its notification_type field');
    if (this.persistent === undefined) throw new Error('Cannot create a new Notification without its persistent field');
    if (this.recipients === undefined) throw new Error('Cannot create a new Notification without its recipients field');
    if (this.content === undefined) throw new Error('Cannot create a new Notification without its content field');
    ...
    try {
      ...
      const record: EntryRecord<Notification> = await this.notificationsStore.client.createNotification(notification);

      this.dispatchEvent(new CustomEvent('notification-created', {
        composed: true,
        bubbles: true,
        detail: {
