...
export class NotificationDetail extends LitElement {
  ...
  async deleteNotification() {
    try {
      await this.notificationsStore.client.deleteNotification(this.notificationHash);
  ...
  renderDetail(entryRecord: EntryRecord<Notification>) {
    return html`
      <sl-card>
          ...
          <sl-icon-button .src=${wrapPathInSvg(mdiDelete)} @click=${() => this.deleteNotification()}></sl-icon-button>
        </div>
