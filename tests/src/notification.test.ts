...
test('create Notification', async () => {
  await runScenario(async scenario => {
    ...
    const notification: EntryRecord<Notification> = await alice.store.client.createNotification(await sampleNotification(alice.store.client));
...
test('create and read Notification', async () => {
  await runScenario(async scenario => {
    ...
    // Alice creates a Notification
    const notification: EntryRecord<Notification> = await alice.store.client.createNotification(sample);
    assert.ok(notification);
    ...
    const createReadOutput: EntryRecord<Notification> = await toPromise(bob.store.notifications.get(notification.actionHash).entry);
...
test('create and delete Notification', async () => {
  await runScenario(async scenario => {
    ...
    const deletes: Array<SignedActionHashed<Delete>> = await toPromise(bob.store.notifications.get(notification.actionHash).deletes);
